...
function AddAlbumPage() {
    ...
    const onClickSend = async () => {
        if (albumTitle.trim() === '') {
            setError("Название альбома пустое");
            return;
        }

        if (albumArtists.length === 0) {
            setError("У альбома не указаны артисты");
            return;
        }

        for (let index = 0; index < tracksToAdd.length; index++) {
            if (!tracksToAdd[index].title) {
                setError(`У трека под номером ${index + 1} нет названия`);
                return;
            }
        }
            ...
            <div className="add-album-wrapper">
                ...
                <div className="add-album-form">
                    ...
                    <div className="add-album__input">
                        ...
                        <p>Нет нужного артиста? <Link to="/add_artist"><span
                            className="link">Добавить артиста</span></Link></p>
                    </div>
                </div>
